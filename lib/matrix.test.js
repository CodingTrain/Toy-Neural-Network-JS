...
test('matrix duplication', () => {
  let m = new Matrix(5, 5);
  m.randomize();

  let n = Matrix.duplicate(m);

  expect(n).toBeInstanceOf(Matrix)
  expect(n).toEqual({
    rows: m.rows,
    cols: m.cols,
    data: m.data
  });
  expect(m == n).toBeFalsy();
});

test('error handling of matrix duplication when param is not a matrix.', () => {
  //Replace console.log with a jest mock so we can see if it has been called
  global.console.log = jest.fn();

  let m = Matrix.duplicate(5)

  //Check if the mock console.log has been called
  expect(global.console.log).toHaveBeenCalledWith("The param for the \'duplicate\' function should be a matrix.")
});
