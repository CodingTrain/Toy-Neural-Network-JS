...
class NeuralNetwork {
  ...
  static deserialize(data) {
    ...
    let args = [null];
    args.push(data.inputLayer.nodes);
    for(let hidden of data.hiddenLayers) {
      args.push(hidden.nodes);
    }
    args.push(data.outputLayer.nodes);

    let neuralNetwork = new (Function.prototype.bind.apply(this, args));

    neuralNetwork.learningRate = data.learningRate;

    for(let i = 0; i < data.hiddenLayers.length; i++) {
      neuralNetwork.hiddenLayers[i].bias = Matrix.deserialize(data.hiddenLayers[i].bias);
    }
    neuralNetwork.outputLayer.bias = Matrix.deserialize(data.outputLayer.bias);

    for(let i = 0; i < data.weightLayers.length; i++) {
      neuralNetwork.weightLayers[i] = Matrix.deserialize(data.weightLayers[i]);
    }
  ...
  mutate(rate) {
    ...
    for(let weight of this.weightLayers) {
      weight.map(mutate);
    }
    for(let hidden of this.hiddenLayers) {
      hidden.bias.map(mutate);
    }
    this.outputLayer.bias.map(mutate);
  }
}
